...
} from '../../../../renderer/layouts/DefaultLayout/functions/getObjectFromJsonFile';
...
const BlogArticleDataProviderCaptureShowPageFragment = (
  ...
  type CaptureStateDisplayedMediaType =
    ...
    | 'snapshot'
    | 'metadata';
  type CaptureFocusedMediaStateReturnType = {
    mediaIsFocused: boolean;
    displayedMediaType: CaptureStateDisplayedMediaType;
  };
  const { mediaIsFocused, displayedMediaType } =
  ...
  const getFocusedCapturePart = useCallback(() => {
    ...
    if (
      ...
      return null;
    const capturePart = props.capture.captureParts.find(
  ...
  const captureReadabilityObject =
    useAsyncMemo<GetObjectFromJsonFileReturnType>(
      () =>
        getObjectFromJsonFile({
          if:
            props.capture != null &&
            props?.capture?.schedule?.status === 'pending' &&
            mediaIsFocused,
          filePath: () => {
            const focusedCapturePart = getFocusedCapturePart();
            return focusedCapturePart != null
              ? 'marchive-downloads:///capture-part/' +
                  focusedCapturePart.id +
                  '/readability.json'
  ...
  const { captureImageUrl, imageDimensions } =
  ...
  const captureSnapshotUrl = useAsyncMemo<string | null>(
    () =>
      new Promise((resolve, reject) => {
        if (mediaIsFocused === false || displayedMediaType !== 'snapshot')
          return resolve(null);
  ...
  const getAbsoluteDownloadLocationForFocusedCaptureOrCapturePart = () => {
    ...
    if (
      Array.isArray(locationFocusedSearchParam) &&
      locationFocusedSearchParam[0] === 'capture-part' &&
      typeof locationFocusedSearchParam[1] === 'number'
    ) {
      const capturePart = props.capture.captureParts.find(
        (capturePart) => capturePart.id === locationFocusedSearchParam[1]
      );
  ...
  const imageMediaContextMenu = (menuProps: ContextMenuContentProps) => {
    ...
    const openDownloadLocationButton = (
      <MenuItem
        icon="folder-open"
    ...
    return (
      <Menu>
        {typeof captureImageUrl === 'string' &&
          captureImageUrl !== 'error' &&
          openImageButton}
        {typeof captureImageUrl === 'string' &&
          captureImageUrl !== 'error' &&
          openDownloadLocationButton}
        {typeof captureImageUrl === 'string' &&
          captureImageUrl !== 'error' &&
          props.capture.captureParts.length > 1 && <MenuDivider />}
