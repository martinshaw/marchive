  ...
  const state = useAsyncMemo<UsePodcastAudioVideoUrlReturnType>(
    () => {
      return getObjectFromJsonFile({
        ...
        if (capturePart?.dataProviderPartIdentifier === 'audio-item')
          returnValue.mediaType = 'audio-item';
        if (capturePart?.dataProviderPartIdentifier === 'video-item')
          returnValue.mediaType = 'video-item';
        ...
        if (
          ...
          typeof returnValue.capturePartMetadataObject?.contentSnippet ===
            'string' &&
