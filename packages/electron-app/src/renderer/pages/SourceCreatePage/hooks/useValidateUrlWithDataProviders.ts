...
const useValidateUrlWithDataProviders = (url: string) => {
  ...
  const [validDataProviders, setValidDataProviders] = useState<
    DataProviderSerializedType[]
  >([]);
  const [loadingValidDataProviders, setLoadingValidDataProviders] =
  ...
  useEffect(() => {
    ...
    window.electron.ipcRenderer.once(
      ...
      (validDataProvidersValue, error) => {
        ...
        setValidDataProviders(
          validDataProvidersValue as DataProviderSerializedType[],
        );
        setLoadingValidDataProviders(false);
        setErrorMessage(false);
      },
    ...
    return () => {
      window.electron.ipcRenderer.removeAllListeners('data-providers.validate');
    };
  }, [debouncedUrlValue]);
