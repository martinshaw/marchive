...
ipcMain.on(
  ...
  async (event, newValue) => {
    // if (newValue != null) await setStoredSettingValue('SCHEDULE_RUN_PROCESS_IS_PAUSED', newValue);
    event.reply(
      'utilities.schedule-run-process-is-paused',
...
ipcMain.on(
  'utilities.capture-part-run-process-is-paused',
  async (event, newValue) => {
    // if (newValue != null) await setStoredSettingValue('CAPTURE_PART_RUN_PROCESS_IS_PAUSED', newValue);
    event.reply(
      'utilities.capture-part-run-process-is-paused',
