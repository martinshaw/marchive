...
const ScheduleUpdateAction = async (
  scheduleId: number,
  requestedChanges: Partial<Schedule>,
): Promise<Schedule> => {
  ...
  if (typeof requestedChanges.interval !== 'undefined') {
    if (
      ...
      requestedChanges.interval === null &&
      typeof requestedChanges.interval !== 'undefined'
    ) {
      const errorMessage =
    ...
    if (requestedChanges.interval === null) {
      ...
      scheduleChanges.nextRunAt = new Date(
        Date.now() + scheduleChanges.interval * 1000,
      );
    }
  }
  ...
  const updatedSchedule = await Schedule.save<Schedule>({
    ...schedule,
    ...scheduleChanges,
  });
